$(function () {
  ...
  const main_cat = localize.main_cat, // functionに記述したカテゴリのスラッグを取得
    ...
    key_area = $('input[class="keyword__text"]'), // キーワード入力場所
    // 通常のAJAX関数
    ajax_fanc = function () {
      $.when(
        $(window).scrollTop(0),
  ...
  let cat_val, menu_val, area_val, tag_val, key_val, paged, url_val, url_par_val, url_all_val, par_val, main_res, page_url = '';
  ...
  function check_url() { // 関数を作成
    ...
    // キーワードのパラメータを追加
    key_val = url.searchParams.get('fw');
  };
  ...
  function ajax_data() {
    ...
    paged = null,
      // 使う変数を宣言
      cat_val = [], area_val = [], menu_val = [], tag_val = [], par_val = '', url_val = [], url_par_val = '';
    // キーワードの取得
    ...
    $.ajax({
      ...
      .done(function (response) {
        ...
        if (url_par_val != '') {
          ...
          history.pushState({
            page_side: main_res
          }, null, `${path}?${url_par_val}`); // 複数を選択した場合はまだ未完成
